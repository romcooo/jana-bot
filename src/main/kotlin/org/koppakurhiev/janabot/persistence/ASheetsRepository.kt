...
abstract class ASheetsRepository<T>(private val bot: IBot) {
    ...
    protected fun save(sheetId: String, updateCoordinates: String = "A1", values: List<List<String>>): Boolean {
        val service = buildSheetsService()
        val range = ValueRange()
        range.setValues(values)
        val request: Sheets.Spreadsheets.Values.Update =
            service.spreadsheets().values().update(sheetId, updateCoordinates, range)
        request.valueInputOption = "RAW"
        val response = request.execute()
        logger.trace { response }
        return response != null && response.updatedCells != null && response.updatedCells > 0
    }
}